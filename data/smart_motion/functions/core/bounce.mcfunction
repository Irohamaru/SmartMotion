...
execute unless entity @s[scores={sm.Z=-1..1}] if score # sm.Calc matches 0.. store success score @s Z store result entity @s Rotation[0] float 0.01 run scoreboard players remove # Calc 18000
execute unless entity @s[scores={sm.Z=-1..1}] if score # sm.Calc matches ..-1 store success score @s Z store result entity @s Rotation[0] float 0.01 run scoreboard players add # Calc 18000
